...
export class OctokitDescriptionUtil {
    ...
    private async getOptions (files: ModifiedFile[] | SrcFile[], downloadUrl: string, branch: string) {
        ...
        const annotations = await parseFile<Annotation[]>('./out/annotations.json')
            .then(value => value.filter(n => files.some(file => this.toSrc(file)[2] === n.file)))
        ...
        return {
            ...
            downloadUrl,
            count: files.filter(file => (this.isModifiedFile(file) ? file.src : file)[2].endsWith('.uc')).length,
            branch,
            ...
            classes: files.map(file => {
                ...
                const url = `https://github.com/ghostrider-05/RL-dummy-classes2/blob/${branch}/${src[2]}`
                const raw_url = `https://github.com/ghostrider-05/RL-dummy-classes2/raw/refs/heads/${branch}/${src[2]}`
                const changelog = `https://github.com/ghostrider-05/RL-dummy-classes2/blob/main/CHANGELOG.md#${toSnakeCase(name)}`
        
                return `| ${src[0]} | [${name}](${url}) - [raw](${raw_url}) | ${this.isModifiedFile(file) ? file.status : `[view](${changelog})`} |`
    ...
    public async createPullRequest (
        ...
        const downloadUrl = this.getDownloadLink(repo, branch)
        const options = await this.getOptions(files, downloadUrl, branch)
